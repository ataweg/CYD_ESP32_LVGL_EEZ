...
void ili9488_init(void)
{
#if 0
	// From github.com/jeremyjh/ESP32_TFT_library
	// From github.com/mvturnho/ILI9488-lvgl-ESP32-WROVER-B <-- removed
	...
	lcd_init_cmd_t ili_init_cmds[]={
		...
		{ /* 0x36 */ ILI9488_CMD_MEMORY_ACCESS_CONTROL,     {0x28}, 1},
		{ /* 0x3A */ ILI9488_CMD_COLMOD_PIXEL_FORMAT_SET,   {0x66}, 1},
		...
		{ /* 0xB6 */ ILI9488_CMD_DISPLAY_FUNCTION_CONTROL,  {0x02, 0x02}, 2},
		{ /* 0xE9 */ ILI9488_CMD_SET_IMAGE_FUNCTION,        {0x00}, 1},
		{ /* 0x53 */ ILI9488_CMD_WRITE_CTRL_DISPLAY,        {0x28}, 1},
		{ /* 0x51 */ ILI9488_CMD_WRITE_DISPLAY_BRIGHTNESS,  {0x7F}, 1},
...
{
   // ESP_LOGI( TAG, "ili9488_flush w: %d, h: %d", lv_area_get_width(area), lv_area_get_height(area) );
   // ESP_LOGI( TAG, "ili9488_flush x1: %d, y1: %d, x2: %d, y2: %d", area->x1, area->y1, area->x2, area->y2  );

    uint32_t size = lv_area_get_width(area) * lv_area_get_height(area);

    uint16_t *buffer_16bit = (uint16_t *) color_map;
    uint8_t *mybuf;
    do {
   ...
   // ESP_LOGI( TAG, "0x2A 0x%02x 0x%02x 0x%02x 0x%2x",xb[0],xb[1],xb[2],xb[3] );

	/*Page addresses*/
	ili9488_send_cmd(ILI9488_CMD_PAGE_ADDRESS_SET);
	ili9488_send_data(yb, 4);
   // ESP_LOGI( TAG, "0x2B 0x%02x 0x%02x 0x%02x 0x%2x",yb[0],yb[1],yb[2],yb[3] );

	/*Memory write*/
	ili9488_send_cmd(ILI9488_CMD_MEMORY_WRITE);
   // ESP_LOGI( TAG, "0x2C 0x%02x 0x%02x 0x%02x 0x%02x",mybuf[0],mybuf[1],mybuf[2],mybuf[3] );
...
static void ili9488_set_orientation(uint8_t orientation)
{
   ...
   {
   //   MY           MX           MV           BGR
      ( 0 << 7 ) | ( 1 << 6 ) | ( 0 << 5 ) | ( 1 << 3 ),  // 0x48  PORTRAIT
      ( 1 << 7 ) | ( 0 << 6 ) | ( 0 << 5 ) | ( 1 << 3 ),  // 0x88  PORTRAIT_INVERTED
      ( 1 << 7 ) | ( 1 << 6 ) | ( 1 << 5 ) | ( 1 << 3 ),  // 0xE8  LANDSCAPE
      ( 0 << 7 ) | ( 0 << 6 ) | ( 1 << 5 ) | ( 1 << 3 ),  // 0x28  LANDSCAPE_INVERTED
   };
 #endif
#endif
